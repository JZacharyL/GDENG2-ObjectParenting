...
void Bunny::draw(int width, int height)
{
	...
	deviceContext->setIndexBuffer(BunnyMesh->getIndexBuffer());
	deviceContext->setVertexBuffer(BunnyMesh->getVertexBuffer());

	deviceContext->drawIndexedTriangleList(BunnyMesh->getIndexBuffer()->getSizeIndexList(), 0, 0);
}
