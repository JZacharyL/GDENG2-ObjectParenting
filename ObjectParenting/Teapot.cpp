...
Teapot::Teapot(std::string name, PrimitiveType type) : AGameObject(name,  type)
{
	...
	TeapotMesh = (Mesh*)MeshManager::getInstance()->createMeshFromFile(L"C:/Users/monet/source/repos/GDENG2ObjectParent/GDENG2ObjectParent/Assets/objects/teapot.obj");
...
void Teapot::draw(int width, int height)
{
	...
	BrickTex = (Texture*)TextureManager::getInstance()->createTextureFromFile(L"C:/Users/monet/source/repos/GDENG2ObjectParent/GDENG2ObjectParent/Assets/Textures/brick.png");
	constant cc;
	...
	deviceContext->setIndexBuffer(TeapotMesh->getIndexBuffer());
	deviceContext->setVertexBuffer(TeapotMesh->getVertexBuffer());

	deviceContext->drawIndexedTriangleList(TeapotMesh->getIndexBuffer()->getSizeIndexList(), 0, 0);
}
